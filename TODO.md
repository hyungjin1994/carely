...
테스트 117개 통과(`npm test`), 레포 전체 lint 에러 0.
...
**설정할 것은 없다.** 기본값이 관리자 이름이고, 이름은 사람 이름이 확실하므로
받침이 있으면 "이" 를 붙여 나간다 — profiles.name 이 "형진" 이면
"형진이가 좋아하는 음식이 뭘까요?" 로 읽힌다(`nameToLabel`).

`/connect` → 어머니 → 추억 이야기 → 상단 "질문에서 나를 부르는 말" 은 그걸
덮고 싶을 때만 쓴다 — "아들"·"큰딸" 처럼 이름이 아닌 호칭으로 부르게 하거나,
profiles 에 성까지 들어가 있을 때. 그 칸에는 읽히는 그대로 적는다.

> "이" 를 자동으로 붙이는 규칙은 **이름에만** 통한다. 일반 호칭에 적용하면
> "아들" → "아들이" → 조사가 붙어 "아들이가" 가 된다. 그래서 profiles.name
> 에만 적용하고, 관리자가 직접 적은 값은 그대로 쓴다(`lib/korean.ts`).
