...
//     "지식" 1건을 적으면 engine 이 양방향 문제 2개를 파생시킨다.
//     (예: 프랑스/파리 → "프랑스의 수도는?" + "파리 — 어느 나라의 수도일까요?")
//     오답 선택지는 같은 카테고리 풀에서 매 판 새로 뽑으므로 보기 조합이 매번 달라진다.
//     → 문제를 늘리는 가장 싼 방법. 한 줄 추가 = 문제 2개.
//
// (2) QUIZ — 파생이 안 되는 단발 문항 (정답은 o 배열의 0-based 인덱스 a)
//     출제 시 engine 이 보기를 셔플하므로 a 를 어디에 찍든 편향이 생기지 않는다.
//     (셔플 도입 전에는 101문항 중 43개가 a:0 에 몰려 "첫 보기 찍기"가 43% 정답이었다.)
//
// 한 판 안에서 같은 지식이 두 번(양방향 포함) 나오지 않도록 engine 이 출처로 중복을 막는다.
