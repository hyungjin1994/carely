...
--
-- qid 는 문항의 안정적 식별자로 lib/games/quiz-bank.ts 가 만든다.
...
create table public.quiz_seen (
  -- 다른 테이블과 같이 profiles 를 참조한다 (profiles.id 가 auth.users 를 참조).
...
as $$
  -- distinct 필수: 같은 qid 가 두 번 들어오면 ON CONFLICT 가 같은 행을 두 번
  -- 건드려 "cannot affect row a second time" 로 실패한다.
  insert into public.quiz_seen(user_id, qid)
  select distinct auth.uid(), q
